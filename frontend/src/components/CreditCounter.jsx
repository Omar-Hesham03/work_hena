...
function CreditCounter({ onPurchaseClick }) {
    ...
    return (
        <div className="flex items-center gap-1 sm:gap-2 bg-white dark:bg-gray-800 px-1.5 sm:px-3 py-1 sm:py-1.5 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm">
            ...
            {/* First Purchase Badge - Hidden on mobile */}
            {isFirstPurchase && credits <= 10 && (
                <span className="hidden sm:inline text-xs font-semibold text-green-600 dark:text-green-400 bg-green-100 dark:bg-green-900/30 px-2 py-0.5 rounded-full animate-pulse">
                    20% OFF
                </span>
            )}
        </div>
